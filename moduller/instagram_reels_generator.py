...
def _write_individual_reel_outputs(source_name: str, source_blocks: list, data: dict, model_adi: str) -> None:
    ...
    count = 0
    srt_count = 0
    generate_srt = os.getenv("INSTAGRAM_REELS_GENERATE_SRT", "false").strip().lower() in {"true", "1", "yes", "on", "acik", "aktif"}
    for reel in data.get("reel_candidates", []):
    ...
    if generate_srt:
        ...
        logger.info("Tekil reel SRT uretimi ayarlardan kapali oldugu icin atlandi.")
