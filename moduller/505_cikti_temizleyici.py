...
PROFILE_OPTIONS: list[tuple[str, str, str]] = [
    ...
    ("8", "runtime_only", "Sadece state/logs/upload receipt"),
]
...
def _confirm_cleanup(profile_key: str) -> bool:
    ...
    if profile.get("clear_state"):
        print("- state klasoru altindaki uploader state/lock dosyalari")
    if profile.get("clear_logs"):
        print("- logs klasoru altindaki yerel log dosyalari")
    if profile.get("clear_upload_receipts"):
        print("- input/success/failed altindaki youtube_upload_result.json dosyalari")
    print("")
    print("Korunanlar: 00_Inputs, oauth/token dosyalari, kullanici kaynak videolari,")
...
def _clear_log_outputs(log_dir: Path) -> tuple[int, list[str]]:
    cleaned = 0
    warnings: list[str] = []
...
def _clear_upload_receipts(*roots: Path) -> tuple[int, list[str]]:
    cleaned = 0
    warnings: list[str] = []
...
def _run_cleanup_plan(profile_key: str) -> tuple[dict[str, int], list[str]]:
    profile = PROFILE_DEFINITIONS[profile_key]
    warnings: list[str] = []
    details = {
        "outputs": 0,
    ...
    for group in profile.get("output_groups", []):
        cleaned, current_warnings = _clear_directory_contents(_group_output_path(group))
        details["outputs"] += cleaned
        warnings.extend(current_warnings)

    if profile.get("clear_group_json_caches"):
        for group in GROUP_DIR_NAMES:
            cleaned, current_warnings = _clear_directory_contents(_group_json_cache_path(group))
            details["json_caches"] += cleaned
            warnings.extend(current_warnings)

    if profile.get("clear_root_json_cache"):
        cleaned, current_warnings = _clear_directory_contents(ROOT_JSON_CACHE_DIR)
        details["root_json_cache"] += cleaned
        warnings.extend(current_warnings)

    if profile.get("clear_state"):
        cleaned, current_warnings = _clear_directory_contents(BASE_DIR / "state")
        details["state"] += cleaned
        warnings.extend(current_warnings)

    if profile.get("clear_logs"):
        cleaned, current_warnings = _clear_log_outputs(BASE_DIR / "logs")
        details["logs"] += cleaned
        warnings.extend(current_warnings)

    if profile.get("clear_upload_receipts"):
        cleaned, current_warnings = _clear_upload_receipts(
            BASE_DIR / "input",
            BASE_DIR / "success",
            BASE_DIR / "failed",
        )
...
def run() -> dict:
    ...
    details, warnings = _run_cleanup_plan(profile_key)
